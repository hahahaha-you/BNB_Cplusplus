...
void playerCommand::move() {
    ...
    case TURN_RIGHT:
        ...
        if (newCoordinate.first == (int)newCoordinate.first) break;
        if (newCoordinate.first - (int)newCoordinate.first < player->getSpeed()-0.02) newCoordinate.first = (int)newCoordinate.first;
        break;
    case TURN_UP:
    ...
    case TURN_DOWN:
        ...
        if (newCoordinate.second == (int)newCoordinate.second) break;
        if (newCoordinate.second - (int)newCoordinate.second < player->getSpeed() - 0.02) newCoordinate.second = (int)newCoordinate.second;
        break;
    default : break;
    }
    if (currentMap->getBlock(newCoordinate.first, newCoordinate.second)->getType() != ROAD){

        return;
    }
    if (newCoordinate.second != (int)newCoordinate.second) {
        if (currentMap->getBlock(newCoordinate.first, newCoordinate.second + 1)->getType() != ROAD) {
            return;
        }
    }
